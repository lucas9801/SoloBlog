...
完整版本见：[SOLUS 博客完整使用手册](user-guide.md)。
...
这一步会检查站点配置、生产域名、评论/阅读量配置、文章 front matter、重复 slug、发布文章的分类/标签/摘要、封面和正文图片路径、订阅源/sitemap 构建能力等。发布前建议一定先跑。
...
RSS 和 JSON Feed 也会在这里检查，确保订阅源包含全文内容，并且正文里的站内链接使用正式域名的绝对地址。
...
文章页侧栏会自动显示当前专题的阅读列表。`seriesOrder` 必须是正整数，建议从 1 开始递增；同一专题内不要重复。
...
```markdown
> [!NOTE] 假设
> 这个结论基于 Unity 2022 LTS 和移动端前向渲染。

> [!WARNING]
> 不要只依赖编辑器 Profiler，最终数据要以真机为准。
```

支持 `NOTE`、`TIP`、`IMPORTANT`、`WARNING`、`CAUTION`。这类说明块适合标注假设、风险、重点结论和复查条件，比普通引用更适合技术档案。
